...
class AnimalRestControllerTest {
    ...
    void addAnimal_CorrectData_Created() throws Exception {
        ...
        animalResponseDto.setSpecies(animalRequest.getSpecies());

        when(animalService.createAnimal(any(AnimalRequestDto.class))).thenReturn(animalResponseDto);
    ...
    void getAnimal_CorrectData_Created() throws Exception {
        AnimalResponseDto animalResponseDto = new AnimalResponseDto();
        animalResponseDto.setSpecies(SPECIES);
        animalResponseDto.setId(ID);

        when(animalService.getAnimalById(anyLong())).thenReturn(animalResponseDto);

        mockMvc.perform(get("/api/animals/{id}", ID)
                .content(objectMapper.writeValueAsString(animalResponseDto))
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
